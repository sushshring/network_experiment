...
def main(orchestrator_name: str, orchestrator_glob: str, baseline_glob: str, ax: Axes):
    orchestrator = OrchestrationPlatform(
        orchestrator_name, 'quad', orchestrator_glob)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Get effect of orchestrator on control baseline')
    parser.add_argument('orchestrator_glob', metavar='orcglobber', type=str,
                        help='file glob pattern to match all data files for the run with orchestrator')
    parser.add_argument('baseline_glob', metavar='baseglobber', type=str,
                        help='file glob pattern to match all data files for the run without orchestrator')
